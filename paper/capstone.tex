...
The diagram in \ref{keyflow} shows the detailed process on Alice's computer that WireGuard goes through to send a handshake initiation message. First WireGuard reads Alice's private key and Bob's public key, sending them to a key derivation process to output the Handshake precomputed-static key. Next Alice generates an ephemeral key-pair
